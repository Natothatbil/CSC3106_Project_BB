...
| 2 | Ongoing, concentrated credential-guessing against `web01` from a handful of external IPs | **High, and still happening as far as we can tell** | **High** | Four IPs (`203.0.113.77`, `198.51.100.8`, `203.0.113.101`, `198.51.100.140`) account for 2,935 of 3,724 failed attempts, 79% of all of them (`output/top_source_ips.csv`). Two of the four also tripped sshd's own break-in warning 35 times combined, which we didn't have to infer since sshd flagged it. | We have no way of knowing if this is one actor rotating source addresses or several unrelated scanners hitting the same box. |
